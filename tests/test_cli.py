...
def test_parser_requires_output():
    p = build_parser()
    with pytest.raises(SystemExit):
        p.parse_args(["convert", "src"])


def test_parser_accepts_source_and_output():
    p = build_parser()
    args = p.parse_args(["convert", "src", "--output", "out"])
...
def test_parser_custom_fail_threshold():
    p = build_parser()
    args = p.parse_args(["convert", "src", "--output", "out", "--fail-threshold", "0.5"])
...
def test_parser_custom_max_bytes():
    p = build_parser()
    args = p.parse_args(["convert", "src", "--output", "out", "--max-bytes", "1048576"])
    assert args.max_bytes == 1_048_576


def test_parser_dry_run_flag():
    p = build_parser()
    args = p.parse_args(["convert", "src", "--output", "out", "--dry-run"])
...
def test_parser_verbose_flag():
    p = build_parser()
    args = p.parse_args(["-v", "convert", "src", "--output", "out"])
    assert args.verbose is True


def test_parser_quiet_flag():
    p = build_parser()
    args = p.parse_args(["-q", "convert", "src", "--output", "out"])
...
def test_e2e_converts_tree(tmp_path):
    ...
    r = _run_cli("convert", str(src), "--output", str(out))
...
def test_e2e_dry_run_writes_nothing(tmp_path):
    ...
    r = _run_cli("-v", "convert", str(src), "--output", str(out), "--dry-run")
    assert r.returncode == 0, r.stderr
...
def test_e2e_idempotent_rerun(tmp_path):
    ...
    r1 = _run_cli("convert", str(src), "--output", str(out))
    assert r1.returncode == 0
    body1 = (out / "page1.md").read_text(encoding="utf-8")

    r2 = _run_cli("convert", str(src), "--output", str(out))
...
def test_e2e_missing_source_exits_2(tmp_path):
    r = _run_cli("convert", str(tmp_path / "nope"), "--output", str(tmp_path / "out"))
...
def test_e2e_help_lists_all_flags():
    r = _run_cli("convert", "--help")
    assert r.returncode == 0
    for flag in ("--output", "--fail-threshold", "--max-bytes", "--dry-run"):
